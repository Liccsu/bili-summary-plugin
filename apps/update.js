...
export class Update extends plugin {
  constructor () {
    super({
      ...
      rule: [
        {
          reg: '^#(bili|Bili|BILI|b站|B站)(省流|总结)(插件)?(强制)?更新$',
          fnc: 'update'
        },
        {
          reg: '^#(bili|Bili|BILI|b站|B站)(省流|总结)(插件)?更新日志$',
          fnc: 'updateLog'
        }
      ]
    })
  ...
  async getLog (plugin = `${pluginName}`) {
    ...
    let end = `更多详细信息，请前往GitHub查看\nhttps://github.com/Liccsu/${pluginName}`
    log = await common.makeForwardMsg(this.e, [log, end], `${pluginName}更新日志，共${line}条`)
